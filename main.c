...
int main()
{
    ...
    char *usrCredentials[] = {"Create an Account", "Login", "Forget Password", "Delete Account", "Exit"};
    ...
    while (repeat)
    {
        ...
        while (1)
        {
            ...
            if (highlight == 1 && ch == 10)
            {
                ...
                char password[50];
                char scrtKey[50];
                WINDOW *win2 = newwin(yMax / 2, xMax / 2, 10, xMax / 4); // Create a new window
                box(win2, 0, 0);
                clear();
                refresh();
                ...
                if (file != NULL)
                {
                    ...
                    mvwprintw(win2, yMax / 4 - 2, xMax / 5, "  User Already Exists!  ");
                    wrefresh(win2);
                    wattroff(win2, COLOR_PAIR(2) | A_BOLD);
                    wattron(win2, COLOR_PAIR(4));
                    mvwprintw(win2, yMax / 4 + 2, xMax / 5 - 20, "  Please go back to --> Forget password  or --> Delete your Account  ");
                    wrefresh(win2);
                    wattroff(win2, COLOR_PAIR(4));

                    wattron(win2, COLOR_PAIR(3));
                    mvwprintw(win2, yMax / 4 + 8, xMax / 5, "  Press any key to go back  ");
                    wrefresh(win2);
                ...
                {
                    // if user doesn't valid username and password dlelete the  created .txt files
                    remove("users.txt");
                    remove("input.txt");
                    clear();
                    refresh();
                ...
                else
                {
                    ...
                    WINDOW *win5 = newwin(yMax / 3, xMax / 3, yMax / 4, xMax / 4);
                    box(win5, 0, 0);
                    refresh();
                    wrefresh(win5);
                    curs_set(0);
                    ...
                    mvwprintw(win5, 9, xMax / 13, " Please save the Secret Key: %s to Retrieve Your Account !  ", user.scrtKey);
            ...
            {
                clear();
                WINDOW *win2 = newwin(yMax / 4, xMax / 4, yMax / 4, xMax / 3); //  new window
                box(win2, 0, 0);
                refresh();
                start_color();
                init_pair(1, COLOR_GREEN, COLOR_BLACK);
                wattron(win2, COLOR_PAIR(1));
                mvwprintw(win2, yMax / 8, xMax / 13, "  You have Exited Successfully!  ");
                wattroff(win2, COLOR_PAIR(1));
                wrefresh(win2);
                delwin(win2);
                getch();
            ...
            else if (highlight == 3 && ch == 10)
            {
                curs_set(1);
                clear();
                WINDOW *winscrt = newwin(yMax / 6, xMax / 4, yMax / 4, xMax / 3);
                box(winscrt, 0, 0);
                refresh();
                wrefresh(winscrt);
                mvwprintw(winscrt, 4, 1, "  Enter Your Secret Key:  ");
                wrefresh(winscrt);
                char secrtKey[100];
                mvwgetstr(winscrt, 4, 27, secrtKey);
        ...
        {
            char *options[] = {"1.Generate Passowrd  ", "2.Show Passowrd  ", "3.Save Password  ", "4.Remove Passowrd", "5.Go Back To Main Menu  "};
            ...
            while (1)
            {
                ...
                if (choice == 1 && inpt == 10)
                {
                    ...
                    refresh();
                    wrefresh(winX);
                    mvwprintw(winX, yMax / 11, 1, "  Enter Password Length:  ");
                    wrefresh(winX);
                    char passLength[10];
                    mvwgetstr(winX, yMax / 11, 25, passLength);
                    int length = atoi(passLength); // Convert string to integer
                    User pass = generatePassword(length);
                    remove_Newline(pass.password);
                    mvwprintw(winX, yMax / 11 + 2, 1, "  Generated Password:%s ", pass.password);
                    // noecho();
                    wrefresh(winX);
                    wattron(winX, COLOR_PAIR(3) | A_REVERSE);
                    mvwprintw(winX, yMax / 9 + 4, xMax / 11, "  Press any key to go back...  ");
                    wrefresh(winX);
                    wattroff(winX, COLOR_PAIR(3) | A_REVERSE);
                    getch();
                    clear();
                    delwin(winX);
                }
                ...
                else if (choice == 2 && inpt == 10)
                {
                    ...
                    while (repeat_ShowPass)
                    {
                        clear();
                        refresh();
                        winX = newwin(yMax / 4, xMax / 3, yMax / 4, xMax / 5); // For not reassiging the window i got a stupid "Segmentation fault" error
                        box(winX, 0, 0);
                        wrefresh(winX);
                ...
                else if (choice == 4 && inpt == 10)
                {
                    ...
                    {
                        mvwprintw(winY, 5, 1, "  Please enter a valid User name and password. ");
                        wrefresh(winY);
                        getch();
                        clear();
                        refresh();
